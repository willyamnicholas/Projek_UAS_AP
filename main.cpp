...
// =============================================================================
// Gambar teks center horizontal pada Y tertentu
// =============================================================================
...
int main() {
    ...
    // AUDIO SETUP
    // =========================================================================
    // backsound.ogg  → Music (streaming, loop terus)
    // pikirkan_secara_logika.ogg → SoundBuffer (pendek, diputar sekali tiap salah)
    // =========================================================================

    // -- Backsound (loop) --
    sf::Music bgm;
    if (!bgm.openFromFile("C:/Katla/Katla/backsound.ogg")) {
 
    }
    else {
        bgm.setLooping(true);      // putar ulang terus
        bgm.setVolume(60.f);     // volume 0-100, sesuaikan selera
        bgm.play();              // langsung mulai dari MENU
    }

    // -- Suara tebakan salah (sekali putar) --
    sf::SoundBuffer bufSalah;
    sf::Sound sfxSalah(bufSalah);  // deklarasi awal
    bool sfxSalahLoaded = false;

    if (bufSalah.loadFromFile("C:/Katla/Katla/pikirkan_secara_logika.ogg")) {
        sfxSalahLoaded = true;
        sfxSalah.setVolume(80.f);  // volume 0-100, sesuaikan selera
    }
    else {
        std::cout << "Warning: Gagal memuat pikirkan_secara_logika.ogg" << std::endl;
    }

    // Helper lambda: putar sfx salah (hanya jika berhasil dimuat)
    auto playSalah = [&]() {
        if (sfxSalahLoaded) {
            sfxSalah.stop();   // stop dulu kalau masih bunyi dari tebakan sebelumnya
            sfxSalah.play();
        }
        };
    ...
    // Sprites RESULT
    const float YOUWIN_IMG_W = 530.f; const float YOUWIN_IMG_Y = 75.f;
    const float GAMEOVER_IMG_W = 850.f; const float GAMEOVER_IMG_Y = 40.f;
    ...
    std::vector<std::string> bankKata = {
        ...
        "ILAHI", "ILHAM", "IMBAL", "IMPAS", "INDAH", "INGAT", "INGIN",
                ...
                if (const auto* te = event->getIf<sf::Event::TextEntered>()) {
                    if (te->unicode < 128) {
                        ...
                        if (c == '\b' && !tebakanSekarang.empty())
                            ...
                            bool selesai = false;
                            if (tebakanSekarang == kataRahasia) {
                                menang = true;
                                selesai = true;
                            }
                            else if (daftarTebakan.size() >= 6) {
                                menang = false;
                                selesai = true;
                            }
                            else {
                                // Tebakan salah & belum habis → putar sfx
                                playSalah();
                            }
                            tebakanSekarang = "";
                            if (selesai) {
